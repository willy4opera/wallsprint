...
if(empty($size)|| $size == 'custom') $size = array(450,450);
...
if(empty($item_meta_select)) {
    $item_meta_select = ['author','comments'];
}
...
<?php echo '<div '.$item_inner.'>';?>
   <?php if($item_thumbnail == 'yes' && (has_post_thumbnail() || !empty($thumbnail_default["id"]))):?>
        <div class="post-thumb <?php echo esc_attr($thumbnail_animation_default)?>">
            ...
            <?php  echo '<span class="date title26"><span class="day font-semibold">'.get_the_date('d').'</span><span class="th">'.get_the_date('M').'</span></span>'; ?>
        </div>
    <?php endif?>   
    <div class="post-info">
        <?php if($item_title == 'yes'):?><h3 class="title20 post-title font-medium"><a class="color-title" href="<?php echo esc_url(get_the_permalink()) ?>"><?php the_title()?></a></h3><?php endif ?>

        <?php if($item_meta == 'yes') bzotech_display_metabox('detail-post',$item_meta_select,'','meta-post-style4');?>
        <?php if($item_excerpt == 'yes') echo '<p class="desc title16">'.bzotech_substr(get_the_excerpt(),0,$excerpt).'</p>';?>
