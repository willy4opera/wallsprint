...
?>
		...
		<?php if ($item_thumbnail == 'yes'): ?>
			<div class="product-thumb <?php echo esc_attr($class_attribute); ?> product">
				...
				if ($item_gallery_hover == 'yes' && !empty($attachment_ids) && is_array($attachment_ids)) {
					...
					<?php } else if (($animation !== 'rotate-thumb' && $animation !== 'translate-thumb')) { ?>
				...
				<?php if ($item_price == 'yes') { echo '<div class="item-price flex-wrapper align-content-center">'; if($product->get_type() == 'simple') echo '<span>'.esc_html__('From','bw-printxtore').'</span>'; bzotech_get_price_html(); } echo '</div>';?>
				<div class="product-extra-link">
					<?php if (class_exists('YITH_WCWL')) { echo bzotech_wishlist_url(); } ?>
		...
		<div class="product-info">
			...
			<?php endif?>
			<?php if($item_excerpt == 'yes') echo '<div class="desc">'.bzotech_substr(apply_filters( 'woocommerce_short_description', $post->post_excerpt ),0,$excerpt).'</div>'; ?>
			...
			<?php if ($item_button == 'yes'): ?>
				...
				bzotech_addtocart_link([
					...
					'el_class' => 'elbzotech-bt-global-default justify_content-center',
					'style' => '',
				]);
				?>
			<?php endif?>
			<?php if (!empty($product_type) && $product_type == 'flash_sale' && $item_flash_sale == 'yes') {
				bzotech_flashsale_countdown_and_stock_prod();
			} ?>
		</div>
