				...
				<?php if($item_button == 'yes'):?>
					<div class="action-buttons">
						...
						if(!empty($button_icon['value'])){
							...
							$icon = '';
							$icon_after = '<i class="far fa-shopping-cart"></i>';
						}
						bzotech_addtocart_link([
							'icon'		=>$icon,
							'icon_after'=>$icon_after,
							'el_class'=>'addcart-link-style5',
							'style'=>''
						]);
						?>
			...
			<?php 
				...
				if(!empty($brand) && $item_brand == 'yes'){
					...
					foreach ($brand as $key => $value) { 
						...
						echo '<a class="item-brand-product-image" href="'.$value['link'].'">'.wp_get_attachment_image($value['logo_brand_product'],'full').'</a>';
