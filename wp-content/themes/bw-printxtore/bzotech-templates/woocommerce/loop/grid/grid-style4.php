...
<?php if($view !== 'slider-masory') echo '<div '.$item_wrap.'>';?>
	<?php if($view !== 'slider-masory') echo '<div '.$item_inner.'>';?>
		...
		<?php if($item_thumbnail == 'yes'):?>
			<div class="product-thumb elbzotech-border-radius <?php echo esc_attr($class_attribute); ?> product">
				...
				<div class="product-thumb__group-extra">
					<?php if(class_exists('YITH_WCWL')) echo bzotech_wishlist_url('<i class="la-heart-o la title24" aria-hidden="true"></i>','','wobble-horizontal');?>
					<?php if(class_exists('YITH_Woocompare')) echo bzotech_compare_url('<i class="las la-sync title24"></i>',false,'','wobble-horizontal');?>
					<?php if($item_quickview == 'yes') bzotech_product_quickview('wobble-horizontal')?>					
				</div>
