	...
	<?php if($view !== 'slider-masory') echo '<div '.$item_inner.'>';?>
		...
		<?php if($item_thumbnail == 'yes'):?>
			<div class="product-thumb <?php echo esc_attr($class_attribute); ?> product">
				...
				<?php if($item_label == 'yes') bzotech_product_label(true,'style2')?>
		...
		<div class="product-info">
			<div class="product-info2">
				...
				<?php endif?>
				<div class="rating-shipping flex-wrapper align_items-center justify_content-space-between">
					...
					if( ! is_wp_error($shipping_class_term) && is_a($shipping_class_term, 'WP_Term') ) {
					    echo '<div class="shipping-class '.$shipping_class_term->slug.'"><i class="las title20 la-shipping-fast"></i>'.$shipping_class_term->name.'</div>';
					}
					?>
				</div>
				...
				<div class="price-brand flex-wrapper align_items-center justify_content-space-between">
					<?php 
					
					if($item_price == 'yes'){
						if($product->is_on_sale() && ($product->get_type() == 'simple'||$product->get_type() == 'external')) echo '<div class="price-on_sale"><p class="title16 font-semibold main-color">'.esc_html__('Sale Price','bw-printxtore').'</p>';
						bzotech_get_price_html();
						if($product->is_on_sale() && ($product->get_type() == 'simple'||$product->get_type() == 'external')) echo '</div>';
					} ?>
					<?php 
					$brand = bzotech_get_loop_term_meta('logo_brand_product','brand_woo');
					if(!empty($brand)){
						echo '<div class="flex-wrapper flex_direction-column">';
						foreach ($brand as $key => $value) {
							echo '<a href="'.$value['link'].'">'.wp_get_attachment_image($value['logo_brand_product'],'full').'</a>';
						}
						echo '</div>';
					}
					?>
			...
			<?php if($item_button == 'yes'):?>
				...
				if(!empty($button_icon['value'])){
					...
					$icon = '';
					$icon_after = '<i class="title22 las la-shopping-basket"></i>';
				}
				bzotech_addtocart_link([
					'icon'		=>$icon,
					'icon_after'=>$icon_after,
					'el_class'=>'addcart-link-style1 font-semibold text-uppercase',
					'style'=>''
				]);
				?>
			<?php endif?>			
