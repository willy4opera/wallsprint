    	...
    	<?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
        	...
        	if($post_query->have_posts()) {
                while($post_query->have_posts()) {
                    $post_query->the_post();
                    if($dem ==1 || $dem % 3 == 1) echo '<div class="swiper-slide flex-wrapper flex_wrap-wrap">';
                    if($dem % 3 == 2) echo '<div class="group-right">';
        			bzotech_get_template_post('grid/item-slider2',null,$attr,true);
                    if($dem % 3 == 0 || $dem == 3 || 3 == $count_query) echo '</div></div>';
                    $dem = $dem+1;
        		}
        	}
        	?>
