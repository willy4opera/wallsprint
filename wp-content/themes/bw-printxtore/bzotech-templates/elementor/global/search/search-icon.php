...
if(!empty($key_trending)){
	...
	$trending_html .= '<div class="key-trending">
        			<h3 class="key-trending-title title16 font-bold">'.$title_trending.'</h3><div class="key-trending-list">';
 ...
 $class_cate = 'cate_hide';
if($show_cat == 'yes' && $search_in != 'all') $class_cate = 'cate_show';
?>
<div class="elbzotech-search-wrap-global elbzotech-search-global-icon-popup <?php echo 'elbzotech-search-global-'.esc_attr($settings['style'].' live-search-'.$live_search)?>">
	...
	<div class="elbzotech-search-form-wrap-global <?php echo esc_attr($class_cate);?>">
		...
		<div class="content-form-popup">			
			<form class="elbzotech-search-form-global" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<div class="wap-content-search-icon-popup">
					<?php echo '<h3 class="text-center title34 font-bold font-title title-form-icon-popup">'.$title_form_icon.'</h3>';  ?>
					<div class="wap-content-search-icon-popup-form flex-wrapper align_items-center flex-align-stretch">
		       		...
		       		<?php echo apply_filters('bzotech_output_trending_html',$trending_html);?>
		       	</div>
		        <?php
		        ...
		        echo '<div class="js-list-live-search elbzotech-list-product-search box-live-e display-'.$display.'" data-load="'.esc_attr($data_loadjs).'">';
            		if($search_in == 'product' && $display == 'grid'){
				        echo '<div class="list-search-default">
						        ...
						        <div '.$wrapper.'><h3 class="search-popular-title title24 font-bold text-uppercase">'.$title_live_default.'</h3><div '.$inner.'>';
				        		...
				        		echo '</div></div>';
				        echo '</div>';
				        echo '<div '.$wrapper.'><h3 class="search-results-title title24 font-bold text-uppercase">'.esc_html__('Product Results','bw-printxtore').'</h3>';
