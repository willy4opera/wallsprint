    ...
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            ...
            foreach (  $list_testimonial3 as $key => $item ) {
                ...
                $wdata->add_render_attribute( 'elbzotech-item-slider-'.$style.'-'.$key, 'class', 'text-center swiper-slide wslider-item elementor-repeater-item-'.$item['_id'] );
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item-slider-'.$style.'-'.$key ).'><div class="item-slider-global-'.$style.'">';
                        ...
                        echo '<div class="info-client text-center">';
                            echo '<div class="content-slider-custom box-content-custom title20 color-title">'.bzotech_parse_text_editor( $item['content']).'</div>';
                            echo '<div class="name-info flex-wrapper align_items-center justify_content-center">';
                            if(!empty($item['title'])) echo '<h3 class="title title18 font-semibold">'.$item['title'].'</h3>';
                               
                            if(!empty($item['description'])) echo '<p class="desc title18"> - '.$item['description'].'</p>';
                        echo '</div>';
                    echo '</div>';
