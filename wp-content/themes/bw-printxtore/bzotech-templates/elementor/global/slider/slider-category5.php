...
<?php echo '<div ' . $wdata->get_render_attribute_string('elbzotech-wrapper') . '>'; ?>
    ...
    <?php echo '<div ' . $wdata->get_render_attribute_string('elbzotech-wrapper-slider') . '>'; ?>
        <?php echo '<div ' . $wdata->get_render_attribute_string('elbzotech-inner') . '>'; ?>
            <?php
foreach ($list_categories5 as $key => $item) {
	...
	echo '<div ' . $wdata->get_render_attribute_string('elbzotech-item-slider-' . $style . '-' . $key) . '><div class="  item-slider-global-' . $style . ' zoom-image">';
	...
	if (!empty($item['link']['url'])) {
		...
		if ($item['link']['is_external']) {
			$wdata->add_render_attribute('data_link' . $key, 'target', "_blank");
		}

		if ($item['link']['nofollow']) {
			$wdata->add_render_attribute('data_link' . $key, 'rel', "nofollow");
		}
	...
	if (!empty($item['template'])) {
		...
		echo '<a class="cate-item text-center " ' . $link_html . '><span class="adv-thumb-link" >' . $image_html . ' </span>
                            <h2 class="info-title color-white bg-color font-medium title20">' . $title_html . '</h2></a>';
	}
