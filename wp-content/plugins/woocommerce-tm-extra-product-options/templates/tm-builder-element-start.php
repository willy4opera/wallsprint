...
if ( isset( $repeater, $repeater_quantity, $repeater_min_rows, $repeater_max_rows, $haslogic, $logic, $description, $class_id, $required, $uniqid, $column, $element_type, $description_position, $label, $label_position, $label_size, $divider, $divider_class, $rules, $original_rules, $rules_type ) ) :
	...
	$divclass = [ 'tc-container', 'tc-container-enabled', 'cpf-element', 'tc-cell', 'cpf-type-' . $element_type, $column, $tm_product_id_class, $extra_class ];
	...
	if ( isset( $operation_mode ) ) {
		if ( ( 'dynamic_product_price' === $operation_mode && '' === $label && '' === $description ) || ( ! empty( $hide ) && '' === $label && '' === $description ) ) {
			$divclass[] = 'tc-hidden';
		}
	}
	...
	<div class="tc-row">
		...
		if ( 'divider' !== $element_type ) {
			...
			if ( ! empty( $label_color ) ) {
				$css              .= '.tm-epo-element-label.color-' . esc_attr( themecomplete_sanitize_hex_color_no_hash( $label_color ) ) . '{color:' . esc_attr( themecomplete_sanitize_hex_color( $label_color ) ) . ';}';
				$descriptionclass .= ' color-' . themecomplete_sanitize_hex_color_no_hash( $label_color );
			}
			if ( ! empty( $description_color ) ) {
				$css              .= '.tm-element-description.color-' . esc_attr( themecomplete_sanitize_hex_color_no_hash( $description_color ) ) . '{color:' . esc_attr( themecomplete_sanitize_hex_color( $description_color ) ) . ';}';
