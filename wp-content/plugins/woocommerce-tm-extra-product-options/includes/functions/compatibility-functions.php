 ...
 * or work differently accross various WooCommerce versions.
 *
 ...
 * @param boolean $strip stripcslashes or not.
 * @return int
 */
function tc_strlen( $str = '', $strip = false ) {
	if ( function_exists( 'mb_strlen' ) && function_exists( 'mb_check_encoding' ) ) {
	...
	$str = THEMECOMPLETE_EPO_HELPER()->utf8_decode( $str );
	if ( $strip ) {
		$str = stripcslashes( $str );
	}
	return strlen( $str );
...
if ( ! function_exists( 'str_starts_with' ) ) {
	...
	function str_starts_with( string $haystack, string $needle ) {
		return 0 === tc_strlen( $needle ) || 0 === strpos( $haystack, $needle );
	}
}
...
if ( ! function_exists( 'str_ends_with' ) ) {
	...
	function str_ends_with( string $haystack, string $needle ) {
		return 0 === tc_strlen( $needle ) || substr( $haystack, -tc_strlen( $needle ) ) === $needle;
	}
}
...
if ( ! function_exists( 'str_contains' ) ) {
	...
	function str_contains( $haystack, $needle ) {
		return 0 === tc_strlen( $needle ) || false !== mb_strpos( $haystack, $needle );
	}
}
...
if ( ! function_exists( 'themecomplete_get_id' ) ) {
	...
	function themecomplete_get_id( $product ) {
		if ( $product instanceof WC_Product && is_callable( [ $product, 'get_id' ] ) && is_callable( [ $product, 'get_parent_id' ] ) ) {
...
if ( ! function_exists( 'themecomplete_get_variation_id' ) ) {
	...
	function themecomplete_get_variation_id( $product ) {
		if ( $product instanceof WC_Product && is_callable( [ $product, 'get_id' ] ) ) {
			return $product->get_id();
		}
...
if ( ! function_exists( 'themecomplete_get_tax_class' ) ) {
	...
	function themecomplete_get_tax_class( $product, $context = false ) {
		if ( $product instanceof WC_Product && is_callable( [ $product, 'get_tax_class' ] ) ) {
			if ( false !== $context ) {
	...
	function themecomplete_maybe_unserialize( $serialized_string = '' ) {
		...
		if ( false === $unserialized_string && preg_match( '/^[aOs]:/', $serialized_string ) ) {
			$serialized_string = preg_replace_callback(
				'/s\:(\d+)\:\"(.*?)\";/su',
				function ( $matches ) {
					return 's:' . tc_strlen( $matches[2] ) . ':"' . $matches[2] . '";';
				},
				$serialized_string
			);
