...
function fpd_admin_copy_file( $file_url, $destination_dir ) {
	...
	try {
		...
		if( !function_exists('copy') ) {
			...
			if( !copy( $file_url, $destination_dir . '/' . $filename ) ) {
			...
			if ($content === false) {
				throw new Exception('Failed to get file content.');
			}
			...
			if($bytes !== false) {
				...
				throw new Exception( 'fwrite(): Failed to write content to file.' );
			}
		...
		throw new Exception( $e->getMessage() . '<br>' . ($last_error && isset($last_error['message']) ? $last_error['message'] : '') );
	}
