...
if( !class_exists('FPD_Admin_Scripts_Styles') ) {
	...
	class FPD_Admin_Scripts_Styles {
		...
		public function global_admin_head () {
			...
			<style>
				#toplevel_page_fancy_product_designer .wp-submenu a[href="admin.php?page=fpd_status"] {
		...
		public function enqueue_styles_scripts( $hook ) {
		    ...
		    if( $hook == 'fancy-product-designer_page_fpd_products' || ($disbale_help_context && $hook == 'toplevel_page_fancy_product_designer') ) {
		    ...
		    if( $hook == 'fancy-product-designer_page_fpd_product_builder' ) {
				...
				$products_length = FPD_Product::count_catalog_products();								
				
				$script_options['templates_directory'] = plugins_url('/assets/templates/', FPD_PLUGIN_ROOT_PHP );
				$script_options['products'] = $products_length > intval(fpd_get_option('fpd_disable_products_loading_count')) ? array() : FPD_Resource_Products::get_products( array('limit' => -1) );
				$script_options['disable_products'] = $products_length > intval(fpd_get_option('fpd_disable_products_loading_count')) ? 1 : 0;
			...
			if( $hook == 'fancy-product-designer_page_fpd_settings') {
				...
				wp_localize_script( 'fpd-react-settings', 'fpd_settings_opts', array(
					...
					'configs' => array(
						...
						'genius_url' => Fancy_Product_Designer::LOCAL ? 'http://localhost:3000/' : 'https://fpd-service-router-dcbb17b083ce.herokuapp.com/',
					),
				) );
			...
			//utilities
			if( $hook == 'fancy-product-designer_page_fpd_utilities') {
				...
				wp_enqueue_script( 'fpd-status-tools', plugins_url('/js/utilities.js', __FILE__), false, $version );
			...
			?>
				<script type="text/javascript">
					...
					document.addEventListener('DOMContentLoaded', () => {
						const supportCenterLink = document.querySelectorAll('.wp-submenu a[href="fpd_support_center"]');
						
						if(supportCenterLink?.length) {
							
							supportCenterLink[0].addEventListener('click', (evt) => {
								evt.preventDefault();
								window.open('https://support.fancyproductdesigner.com/', '_blank'); 
		...
		public function global_admin_footer () {
			...
			if( !fpd_get_option('fpd_disable_help_context') ):
				?>
				<script>
					(function (w, d, s) {
