							...
							setTimeout( function() {
								...
								if ( jQuery( '#' + textareaID ).data( 'placeholder' ) ) {
									window.tinyMCE.get( textareaID ).on( 'focus', function( event ) {
										...
										if ( $theContent === jQuery( '#' + textareaID ).data( 'placeholder' ) && ( ( ! textareaHadFocus && ! switchButtonInputHadFocus ) || ( textareaHadFocus && switchButtonInputHadFocus ) ) ) {
			...
			initRepeater: function( $element ) {
				...
				if ( 'object' === typeof values && rows ) {		
					_.each( values, function( field, index ) {
						var newRowTitle = 'undefined' !== typeof values[ index ][ option.bind_title ] && values[ index ][ option.bind_title ] ? values[ index ][ option.bind_title ] : rowTitle;

						const titleField = 'undefined' !== typeof option.fields[ option.bind_title ] ? option.fields[ option.bind_title ] : false;
						if ( '' !== newRowTitle && 'object' === typeof titleField && 'select' === titleField.type && ( 'object' === typeof titleField.choices || 'object' === typeof titleField.value ) ) {
							newRowTitle = titleField.value ? titleField.value[ newRowTitle ] : newRowTitle;
						}	
		
						newRowTitle = 'undefined' !== typeof option && 'undefined' !== typeof option.title_prefix ? option.title_prefix + ' ' + newRowTitle : newRowTitle;
						
						self.createRepeaterRow( fields, values[ index ], $target, newRowTitle );
					} );
				}
