		...
		FusionPageBuilder.MultiElementSortableChild = window.wp.Backbone.View.extend( {
			...
			updatePreview: function() {
				...
				if ( 'undefined' !== typeof $attributes ) {
					...
					} else if ( 'fusion_image_hotspot_point' === $attributes.element_type && 'undefined' !== typeof $attributes.params.icon && $attributes.params.icon && ( 'undefined' === typeof $attributes.params.title || '' === $attributes.params.title ) ) {
						$title = '<i class="awb-preview-icon ' + $attributes.params.icon + '"></i>';
					...
					} else if ( 'fusion_testimonial' !== $model.get( 'element_type' ) &&'undefined' !== typeof $attributes.params.image && $attributes.params.image.length ) {
					...
					if ( -1 === $title.indexOf( 'awb-preview-icon' ) ) {
