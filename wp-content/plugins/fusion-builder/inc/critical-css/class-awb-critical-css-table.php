...
class AWB_Critical_CSS_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		$remove_actions = [ 'action', 'action2', 'post', '_wpnonce' ];
	...
	public function column_css_key( $item ) {
		$wpnonce = wp_create_nonce( 'awb-critical-css-nonce-' . $item['id'] );
		$markup  = $item['css_key'];
		...
		$actions['delete'] = sprintf( '<a href="' . $this->url . '&action=%s&post=%s&_wpnonce=%s">' . esc_html__( 'Delete', 'fusion-builder' ) . '</a>', 'delete_css', esc_attr( $item['id'] ), $wpnonce );
	...
	public function column_desktop_css( $item ) {
		$wpnonce          = wp_create_nonce( 'awb-critical-css-nonce-' . $item['id'] );
		$actions['clear'] = sprintf( '<a href="' . $this->url . '&action=%s&post=%s&_wpnonce=%s">' . esc_html__( 'Clear', 'fusion-builder' ) . '</a>', 'clear_desktop_css', esc_attr( $item['id'] ), $wpnonce );

		$actions['update'] = sprintf( '<a href="' . $this->url . '&action=%s&post=%s&_wpnonce=%s" class="awb-regenerate-desktop-css">' . esc_html__( 'Regenerate', 'fusion-builder' ) . '</a>', 'update_desktop_css', esc_attr( $item['id'] ), $wpnonce );
	...
	public function column_mobile_css( $item ) {
		$wpnonce          = wp_create_nonce( 'awb-critical-css-nonce-' . $item['id'] );
		$actions['clear'] = sprintf( '<a href="' . $this->url . '&action=%s&post=%s&_wpnonce=%s">' . esc_html__( 'Clear', 'fusion-builder' ) . '</a>', 'clear_mobile_css', esc_attr( $item['id'] ), $wpnonce );

		$actions['update'] = sprintf( '<a href="' . $this->url . '&action=%s&post=%s&_wpnonce=%s" class="awb-regenerate-mobile-css">' . esc_html__( 'Regenerate', 'fusion-builder' ) . '</a>', 'update_mobile_css', esc_attr( $item['id'] ), $wpnonce );
