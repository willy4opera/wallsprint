...
// Don't duplicate me!
	if ( ! class_exists( 'FusionReduxFramework_info' ) ) {

		/**
		 * Main FusionReduxFramework_info class
		 *
		 * @since       1.0.0
		 */
		class FusionReduxFramework_info {
			...
			public function render() {
				...
				if ( empty( $this->field['raw_html'] ) ) {
					if ( $this->field['notice'] == true ) {
						$this->field['class'] .= ' fusionredux-notice-field';
					} else {
						$this->field['class'] .= ' fusionredux-info-field';
					}



					$this->field['style'] = 'fusionredux-' . $this->field['style'] . ' ';
				}
				...
				if ( ! empty( $this->field['raw_html'] ) && $this->field['raw_html'] ) {
					...
					if ( isset( $this->field['icon'] ) && ! empty( $this->field['icon'] ) && $this->field['icon'] !== true ) {
						echo '<p class="fusionredux-info-icon"><i class="' . esc_attr($this->field['icon']) . ' icon-large"></i></p>';
					}
					...
					if ( ! empty( $this->field['title'] ) || ! empty( $this->field['desc'] ) ) {
						echo '<p class="fusionredux-info-desc">' . wp_kses_post($this->field['title']) . wp_kses_post($this->field['desc']) . '</p>';
					}
				}
			...
			public function enqueue() {
				if ($this->parent->args['dev_mode']) {
					wp_enqueue_style(
						'fusionredux-field-info-css',
						FusionReduxFramework::$_url . 'inc/fields/info/field_info.css',
						array(),
						time(),
						'all'
					);
