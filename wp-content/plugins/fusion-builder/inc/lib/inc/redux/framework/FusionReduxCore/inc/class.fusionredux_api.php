<?php
	...
	/**
	 * FusionRedux Framework API Class
	 * Makes instantiating a FusionRedux object an absolute piece of cake.
	 *
	...
	if ( ! class_exists( 'FusionRedux' ) ) {
		 ...
		 * Simple API for FusionRedux Framework
		 *
		 * @since       1.0.0
		 */
		class FusionRedux {
			...
			public static function load() {
				add_action( 'after_setup_theme', array( 'FusionRedux', 'createFusionRedux' ) );
				add_action( 'init', array( 'FusionRedux', 'createFusionRedux' ) );
				add_action( 'switch_theme', array( 'FusionRedux', 'createFusionRedux' ) );
			}

			public static function init( $opt_name = "" ) {
				if ( ! empty( $opt_name ) ) {
					self::loadFusionRedux( $opt_name );
					remove_action( 'setup_theme', array( 'FusionRedux', 'createFusionRedux' ) );
				}
			}

			public static function loadExtensions( $FusionReduxFramework ) {
				if ( $instanceExtensions = self::getExtensions( $FusionReduxFramework->args['opt_name'], "" ) ) {
					foreach ( $instanceExtensions as $name => $extension ) {
						if ( ! class_exists( $extension['class'] ) ) {
							// In case you wanted override your override, hah.
							$extension['path'] = apply_filters( 'fusionredux/extension/' . $FusionReduxFramework->args['opt_name'] . '/' . $name, $extension['path'] );
							if ( file_exists( $extension['path'] ) ) {
								require_once wp_normalize_path( $extension['path'] );
							}
						}
						if ( ! isset( $FusionReduxFramework->extensions[ $name ] ) ) {
							if ( class_exists( $extension['class'] ) ) {
								$FusionReduxFramework->extensions[ $name ] = new $extension['class']( $FusionReduxFramework );
							} else {
			...
			public static function extensionPath( $extension, $folder = true ) {
				if ( ! isset( FusionRedux::$extensions[ $extension ] ) ) {
					return;
				}
				$path = end( FusionRedux::$extensions[ $extension ] );
				if ( ! $folder ) {
					return $path;
				}
			...
			public static function loadFusionRedux( $opt_name = "" ) {

				if ( empty( $opt_name ) ) {
					return;
				}

				$check = FusionReduxFrameworkInstances::get_instance( $opt_name );
				...
				if ( ! class_exists( 'FusionReduxFramework' ) ) {
					echo '<div id="message" class="error"><p>FusionRedux Framework is <strong>not installed</strong>. Please install it.</p></div>';

					return;
				}
				if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
					add_action( "fusionredux/extensions/{$opt_name}/before", array( 'FusionRedux', 'loadExtensions' ), 0 );
				}

				$fusionredux                   = new FusionReduxFramework( $sections, $args );
				$fusionredux->apiHasRun        = 1;
				self::$init[ $opt_name ] = 1;
				if ( isset( $fusionredux->args['opt_name'] ) && $fusionredux->args['opt_name'] != $opt_name ) {
					self::$init[ $fusionredux->args['opt_name'] ] = 1;
				}

			}

			public static function createFusionRedux() {
			...
			public static function setSections( $opt_name = '', $sections = array() ) {
				...
				if ( ! empty( $sections ) ) {
					foreach ( $sections as $section ) {
						FusionRedux::setSection( $opt_name, $section );
					}
				}
			}
			...
			public static function getOption ($opt_name = "", $key = "") {
				...
				if (!empty($opt_name) && !empty($key)) {
					$fusionredux = get_option($opt_name);

					if (isset($fusionredux[$key])) {
						return $fusionredux[$key];
					} else {
						return;
					}
				} else {
			...
			public static function setOption ($opt_name = "", $key = "", $option = "") {
				...
				if (!empty($opt_name) && !empty($key)) {
					$fusionredux          = get_option($opt_name);
					$fusionredux[$key]    = $option;

					return update_option($opt_name, $fusionredux);
				} else {
					return false;
				}
			}
			...
			public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
				if ( file_exists( $class_file ) ) {
					...
					$version                   = FusionRedux_Helpers::get_template_version( $class_file );
			...
			public static function getAllExtensions() {
				...
				foreach ( $fusionredux as $instance ) {
					if ( ! empty( self::$uses_extensions[ $instance['args']['opt_name'] ] ) ) {
						continue;
					}
					if ( ! empty( $instance['extensions'] ) ) {

						FusionRedux::getInstanceExtensions( $instance['args']['opt_name'], $instance );
					}
				}
			}
			...
			public static function getInstanceExtensions( $opt_name, $instance = array() ) {
				...
				if ( empty( $instance ) ) {
					$instance = FusionReduxFrameworkInstances::get_instance( $opt_name );
				}
				...
				foreach ( $instance->extensions as $name => $extension ) {
					if ( $name == "widget_areas" ) {
						$new = new FusionRedux_Widget_Areas( $instance );
					}
					if ( isset( self::$uses_extensions[ $opt_name ][ $name ] ) ) {
						continue;
					}
					if ( isset( $extension->extension_dir ) ) {
						FusionRedux::setExtensions( $opt_name, str_replace( $name, '', $extension->extension_dir ) );

					} else if ( isset( $extension->_extension_dir ) ) {
						FusionRedux::setExtensions( $opt_name, str_replace( $name, '', $extension->_extension_dir ) );
					}
				}
			}
			...
			public static function getExtensions( $opt_name = "", $key = "" ) {
				...
				if ( empty( $opt_name ) ) {
					...
					foreach ( self::$uses_extensions[ $opt_name ] as $extension ) {
						...
						$extension_class                  = 'FusionReduxFramework_Extension_' . $name;
