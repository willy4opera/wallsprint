...
// Don't duplicate me!
if ( !class_exists ( 'FusionReduxFramework_slides' ) ) {

	/**
	 * Main FusionReduxFramework_slides class
	 *
	 * @since       1.0.0
	 */
	class FusionReduxFramework_slides {
		...
		public function render () {
			...
			$defaults = array(
				...
				'content_title' => __ ( 'Slide', 'Avada' )
			);

			$this->field = wp_parse_args ( $this->field, $defaults );

			echo '<div class="fusionredux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'Avada' ), $this->field[ 'content_title' ] ) ) . '">';
			...
			if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<div class="fusionredux-slides-accordion-group"><fieldset class="fusionredux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="fusionredux-slides-header">' . $slide[ 'title' ] . '</span></h3><div>';
					...
					echo '<div class="fusionredux_slides_add_remove">';

					echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'Avada' ) . '</span>';
					...
					echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'Avada' ) . '</span>';

					echo '</div>' . "\n";

					echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="fusionredux-slides-list">';
					...
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'Avada' );
					...
					if ( $this->field[ 'show' ][ 'description' ] ) {
						$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'Avada' );
					...
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'Avada' );
					...
					echo '<li><a href="javascript:void(0);" class="button deletion fusionredux-slides-remove">' . __ ( 'Delete', 'Avada' ) . '</a></li>';
			...
			if ( $x == 0 ) {
				echo '<div class="fusionredux-slides-accordion-group"><fieldset class="fusionredux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="fusionredux-slides-header">' . esc_attr ( sprintf ( __ ( 'New %s', 'Avada' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
				...
				echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'Avada' ) . '</span>';
			...
			echo '</div><a href="javascript:void(0);" class="button fusionredux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'Avada' ), $this->field[ 'content_title' ] ) . '</a><br/>';
		}
		...
		public function enqueue () {
			...
			if ($this->parent->args['dev_mode']){
				wp_enqueue_style ('fusionredux-field-media-css');

				wp_enqueue_style (
					'fusionredux-field-slides-css',
					FusionReduxFramework::$_url . 'inc/fields/slides/field_slides.css',
					array(),
					time (),
					'all'
				);
			...
			wp_enqueue_script(
				'fusionredux-field-media-js',
				FusionReduxFramework::$_url . 'assets/js/media/media' . FusionRedux_Functions::isMin() . '.js',
