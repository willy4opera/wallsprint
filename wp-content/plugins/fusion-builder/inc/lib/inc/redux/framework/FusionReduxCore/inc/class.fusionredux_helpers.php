...
// Don't duplicate me!
	if ( ! class_exists( 'FusionRedux_Helpers' ) ) {

		/**
		 * FusionRedux Helpers Class
		 * Class of useful functions that can/should be shared among all FusionRedux files.
		 *
		 * @since       1.0.0
		 */
		class FusionRedux_Helpers {
			...
			public static function tabFromField( $parent, $field ) {
				foreach ( $parent->sections as $k => $section ) {
					...
					if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
						if ( FusionRedux_Helpers::recursive_array_search( $field, $section['fields'] ) ) {
							return $k;
							continue;
						}
					}
...
//                }
			...
			private static function fusionreduxAsPlugin() {
				return FusionReduxFramework::$_as_plugin;
			}
			...
			public static function localize( $localize ) {
				$fusionredux            = FusionReduxFrameworkInstances::get_instance( $localize['args']['opt_name'] );
				$nonce            = wp_create_nonce( 'fusionredux-ads-nonce' );
				$base             = admin_url( 'admin-ajax.php' ) . '?action=fusionredux_p&nonce=' . $nonce . '&url=';
				$localize['rAds'] = FusionRedux_Helpers::rURL_fix( $base, $fusionredux->args['opt_name'] );
			...
			public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
				...
				$sysinfo['fusionredux_ver']      = esc_html( FusionReduxFramework::$_version );
				$sysinfo['fusionredux_data_dir'] = FusionReduxFramework::$_upload_dir;
				$f                         = 'fo' . 'pen';
				// Only is a file-write check
				$sysinfo['fusionredux_data_writeable'] = self::makeBoolStr( @$f( FusionReduxFramework::$_upload_dir . 'test-log.log', 'a' ) );
				...
				if ( ! class_exists( 'Browser' ) ) {
					require_once wp_normalize_path( FusionReduxFramework::$_dir . 'inc/browser.php' );
				}
				...
				if ( $remote_checks == true ) {
					$response = wp_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
						'sslverify'  => false,
						'timeout'    => 60,
						'user-agent' => 'FusionReduxFramework/' . FusionReduxFramework::$_version,
						'body'       => array(
				...
				$sysinfo['fusionredux_instances'] = array();

				if ( ! empty( $fusionredux ) && is_array( $fusionredux ) ) {
					foreach ( $fusionredux as $inst => $data ) {
						FusionRedux::init( $inst );

						$sysinfo['fusionredux_instances'][ $inst ]['args']     = $data->args;
						$sysinfo['fusionredux_instances'][ $inst ]['sections'] = $data->sections;
						foreach ( $sysinfo['fusionredux_instances'][ $inst ]['sections'] as $sKey => $section ) {
							if ( isset( $section['fields'] ) && is_array( $section['fields'] ) ) {
								foreach ( $section['fields'] as $fKey => $field ) {
									if ( isset( $field['validate_callback'] ) ) {
										unset( $sysinfo['fusionredux_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['validate_callback'] );
									}
									if ( $field['type'] == "js_button" ) {
										if ( isset( $field['script'] ) && isset( $field['script']['ver'] ) ) {
											unset( $sysinfo['fusionredux_instances'][ $inst ]['sections'][ $sKey ]['fields'][ $fKey ]['script']['ver'] );
										}
									}
						...
						$sysinfo['fusionredux_instances'][ $inst ]['extensions'] = FusionRedux::getExtensions( $inst );

						if ( isset( $data->extensions['metaboxes'] ) ) {
							$data->extensions['metaboxes']->init();
							$sysinfo['fusionredux_instances'][ $inst ]['metaboxes'] = $data->extensions['metaboxes']->boxes;
						}

						if ( isset( $data->args['templates_path'] ) && $data->args['templates_path'] != '' ) {
							$sysinfo['fusionredux_instances'][ $inst ]['templates'] = self::getFusionReduxTemplates( $data->args['templates_path'] );
						}
					}
				}
			...
			private static function getFusionReduxTemplates( $custom_template_path ) {
				$filesystem         = FusionRedux_Filesystem::get_instance();
				$template_paths     = array( 'FusionReduxFramework' => FusionReduxFramework::$_dir . 'templates/panel' );
				$scanned_files      = array();
				$found_files        = array();
				...
				foreach ( $scanned_files as $plugin_name => $files ) {
					foreach ( $files as $file ) {
						...
						if ( $theme_file ) {
							$core_version  = self::get_template_version( FusionReduxFramework::$_dir . 'templates/panel/' . $file );
							...
							if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
								...
								$found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'Avada' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
							} else {
			...
			public static function get_extension_url( $dir ) {
				$ext_dir = FusionRedux_Helpers::get_extension_dir( $dir );
