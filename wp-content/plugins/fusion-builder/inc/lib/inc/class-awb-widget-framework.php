...
class AWB_Widget_Framework {
	...
	public function __construct() {
		...
		if ( isset( $global_options['status_widget_areas'] ) && '0' === $global_options['status_widget_areas'] ) {
			...
			// Remove side navigation template.
			add_filter( 'theme_templates', [ $this, 'remove_side_navigation_template' ] );
	...
	/**
	 * Add different GO related to widgets and sidebars.
	 *
	 * @since 3.9
	 * @access public
	 * @param string[] $post_templates Array of template header names keyed by the template file name.
	 * @return string[] The adjusted array of template header names keyed by the template file name.
	 */
	public function remove_side_navigation_template( $post_templates ) {
		unset( $post_templates['side-navigation.php'] );
	...
	public function remove_sidebars_page_options( $pagetype_data, $posttype ) {
		if ( isset( $pagetype_data[ $posttype ] ) ) {
			$key = array_search( 'sidebars', $pagetype_data[ $posttype ] ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
		} else {
			$key      = array_search( 'sidebars', $pagetype_data['default'] ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			$posttype = 'default';
		}
	...
	public function add_global_options_sidebar( $sections ) {
		...
		foreach ( $sections as $index => $section ) {
			if ( 11 === $counter && class_exists( 'Avada' ) ) {
				...
				$options = $this->add_side_nav_optionsto_go_extras();

				$section['fields']['misc_options_section']['fields'] = array_merge( $options, $section['fields']['misc_options_section']['fields'] );
	...
	/**
	 * Add layout section options to GO.
	 *
	 * @since 3.9
	 * @access public
	...
	public function set_sidebar_body_classes() {
		...
		$sidebar_1 = ( empty( $sidebar_1 ) || ( isset( $sidebar_1[0] ) && ( empty( $sidebar_1[0] ) || '0' === $sidebar_1[0] ) ) ) ? 'None' : $sidebar_1;
		$sidebar_2 = ( empty( $sidebar_2 ) || ( isset( $sidebar_2[0] ) && ( empty( $sidebar_2[0] ) || '0' === $sidebar_2[0] ) ) ) ? 'None' : $sidebar_2;
		...
		if ( ( Fusion_Helper::is_bbpress() || Fusion_Helper::is_buddypress() ) && ! Fusion_Helper::bbp_is_forum_archive() && ! Fusion_Helper::bbp_is_topic_archive() && ! Fusion_Helper::bbp_is_user_home() && ! Fusion_Helper::bbp_is_search() ) {
	...
	private function sidebar_context( $sidebar = 1 ) {
		...
		if ( Fusion_Helper::is_buddypress() || Fusion_Helper::bbp_is_forum_archive() || Fusion_Helper::bbp_is_topic_archive() || Fusion_Helper::bbp_is_user_home() || Fusion_Helper::bbp_is_search() ) {
	...
	public function sidebar_options() {
		...
		if ( is_home() ) {
			...
			if ( Fusion_Helper::bbp_is_forum_archive() || Fusion_Helper::bbp_is_topic_archive() || Fusion_Helper::bbp_is_user_home() || Fusion_Helper::bbp_is_search() ) {
				$sidebars = [
					'global'    => '1',
					'sidebar_1' => fusion_library()->get_option( 'ppbress_sidebar' ),
					'sidebar_2' => fusion_library()->get_option( 'ppbress_sidebar_2' ),
	...
	public function register_widget_area() {
		...
		register_sidebar(
			[
				...
				'description'   => __( 'Default widget area of Avada', 'Avada' ),
		...
		for ( $i = 1; $i < $columns; $i++ ) {
			...
			register_sidebar(
				[
					/* translators: The footer widget area number. */
					'name'          => sprintf( 'Footer Widget Area %s', $i ),
		...
		for ( $i = 1; $i < $columns; $i++ ) {
			...
			register_sidebar(
				[
					/* translators: The sliding bar widget area number. */
					'name'          => sprintf( 'Sliding Bar Widget Area %s', $i ),
