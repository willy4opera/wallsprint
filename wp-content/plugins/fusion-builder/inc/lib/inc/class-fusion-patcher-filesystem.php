...
class Fusion_Patcher_Filesystem {
	...
	public function get_remote( $url ) {
		...
		new Fusion_Patcher_Admin_Notices( 'no-patch-contents', sprintf( esc_html__( 'The Avada patch contents cannot be retrieved. Please contact your host to unblock the "%s" domain.', 'Avada' ) ), FUSION_UPDATES_URL );
		return false;
	}
	...
	public function write_file() {
		...
		if ( ! $contents ) {
			...
			new Fusion_Patcher_Admin_Notices( 'patch-empty', esc_html__( 'Patch empty.', 'Avada' ) );
			return false;
		}
		...
		if ( false === $target ) {
			...
			$this->status = false;
			// Add a message to users for debugging purposes.
			new Fusion_Patcher_Admin_Notices( 'invalid-patch-target', esc_html__( 'Invalid Patch target.', 'Avada' ) );
			return false;
		}
		...
		if ( ! $this->status ) {
			...
			// Add a message to users for debugging purposes.
			/* translators: Links. */
			new Fusion_Patcher_Admin_Notices( 'write-permissions-' . self::$patch_id, sprintf( __( 'The patch could not be applied because of your specific server permissions. You have two options to remedy this. 1. <a %1$s>Download this zip file</a> which contains the files needed to fix this issue. Simply extract the zip file, and replace the files it contains with the same files on your server. DO NOT REPLACE THE ENTIRE FOLDER. 2. <a %2$s>Contact our support center</a>, submit a ticket and include your FTP credentials so one of our support experts can apply the fix for you. Once the fix is applied, click the "Dismiss Notices" button so this message is removed.', 'Avada' ), 'target="_blank" href="' . $patch_url . '" style="color:#fff;text-decoration:underline;font-weight:bold;"', 'target="_blank" href="https://avada.com/help-center/" style="color:#fff;text-decoration:underline;font-weight:bold;"' ) );
		}
		return $this->status;
	}
}
