...
<script type="text/template" id="fusion-builder-blank-page-template">
	<div class="fusion-builder-blank-page-content fusion-builder-data-cid" data-cid="{{ cid }}">
		...
		<# if ( 'fusion_tb_section' === FusionApp.data.postDetails.post_type ) { #>
			<# if ( FusionApp.data.template_override.page_title_bar && FusionApp.data.postDetails.post_id === FusionApp.data.template_override.page_title_bar.ID ) { #>
				<h1 class="title">{{ fusionBuilderText.to_get_started_ptb }}</h1>
			<# } else if ( FusionApp.data.template_override.footer && FusionApp.data.postDetails.post_id === FusionApp.data.template_override.footer.ID ) { #>
				<h1 class="title">{{ fusionBuilderText.to_get_started_footer }}</h1>
			<# } else if ( FusionApp.data.template_override.header && FusionApp.data.postDetails.post_id === FusionApp.data.template_override.header.ID ) { #>
				<h1 class="title">{{ fusionBuilderText.to_get_started_header }}</h1>
			<# } else if ( 'content' === FusionApp.data.template_category ) { #>
				<h1 class="title">{{ fusionBuilderText.to_get_started_content }}</h1>
			<# } #>
		<# } else if ( 'awb_off_canvas' === FusionApp.data.postDetails.post_type ) { #>
			<h1 class="title">{{ fusionBuilderText.to_get_started_off_canvas }}</h1>
		<# } else if ( 'mega_menus' === FusionApp.data.fusion_element_type ) { #>
			<h1 class="title">{{ fusionBuilderText.to_get_started_mega_menus }}</h1>
		<# } else { #>
	...
	<# if ( 'awb_off_canvas' !== FusionApp.data.postDetails.post_type ) { #>
		...
		<div class="fusion-builder-blank-page-info fusion-builder-blank-page-docs">
			<a href="https://avada.com/documentation/category/builder/" target="_blank" class="info-icon fusion-builder-docs-button">
			...
			<a href="https://avada.com/documentation/category/builder/" target="_blank" class="fusion-builder-submit-button fusion-builder-docs-button">{{ fusionBuilderText.fusion_builder_docs }}<span class="fa-long-arrow-alt-right fas"></span></a>
		</div>
	<# } #>
