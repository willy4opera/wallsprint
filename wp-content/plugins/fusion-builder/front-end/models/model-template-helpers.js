...
_.mixin( {
	...
	buildTitleElement: function( values, extras, headingContent ) {
		var contentAlign = jQuery( 'body' ).hasClass( 'rtl' ) ? 'right' : 'left',
			...
			size = 'div' === values.heading_size || 'p' === values.heading_size ? values.heading_size : 'h' + parseInt( values.heading_size ),
			output = '',
			styles = '',
			headingStyles   = '',
			classes         = '',
		...
		wrapperClasses = ' fusion-title fusion-title-size-' + sizeArray[ size ] + classes;
		...
		output += '<' + size + ' class="title-heading-' + contentAlign + '" style="' + headingStyles + '">';
	...
	fusionGetAnimations: function( args ) {
		...
		if ( args.type ) {
			...
			if ( args.delay ) {
				animationAttributes[ 'data-animationDelay' ] = args.delay;
			}
		}
	 ...
	 * @param {string}         data.display_woo_out_of_stcok - Should we display the Woo out of stock badge?
	 * @param {string}         data.out_of_stock - The Woo out of stock badge (HTML).* * 
	 * @param {string}         data.display_woo_sale - Should we display the Woo sale badge?
	 * @param {string}         data.sale - The Woo sale badge (HTML).* 
	...
	fusionAnimations: function( values, attributes, parent ) {
		...
		if ( values && 'undefined' !== typeof values.animation_type && '' !== values.animation_type ) {
			...
			if ( values.animation_color ) {
				if ( ! attributes.style ) {
					attributes.style = '';
				}
				attributes.style += '--awb-animation-color:' + values.animation_color + ';';
			}
		}
	...
	fusionGetFilterStyleElem: function( values, selector, cid ) {
		var filter_style       = '',
			filter_style_hover = '',
			selectorPrefix     = 'body:not(.fusion-disable-element-filters) ',
			regularSelector    = '',
			hoverSelector      = '';
		...
		if ( '' !== filter_style ) {
			filter_style = selectorPrefix + regularSelector + '{filter: ' + filter_style + ';}';
		}

		filter_style_hover = this.fusionGetFilterStyle( values, 'hover' );
		if ( '' !== filter_style_hover ) {

			if ( 'parent' === values.filter_hover_element ) {
				if ( regularSelector.includes( 'nested-column-' ) ) {
					hoverSelector = '.fusion-column-wrapper:hover ' + regularSelector;
				} else {
					hoverSelector = '.fusion-builder-row:hover > ' + regularSelector;
				}
			}
			...
			// Hover state.
			filter_style += selectorPrefix + hoverSelector + '{filter: ' + filter_style_hover + ';}';
		}
	...
	getFilterVars: function( values ) {
		var filterStyle      = this.fusionGetFilterStyle( values, 'regular' ),
			filterStyleHover = this.fusionGetFilterStyle( values, 'hover' ),
			finalVars        = '';

		if ( '' !== filterStyle ) {
			finalVars += '--awb-filter:' + filterStyle + ';';
		}

		if ( '' !== filterStyleHover ) {
			finalVars += '--awb-filter-transition:filter 0.3s ease;';
			finalVars += '--awb-filter-hover:' + filterStyleHover + ';';

			if ( 'parent' === values.filter_hover_element ) {
				finalVars += '--awb-filter-parent-hover:' + filterStyleHover + ';';
			}
		}
	...
	fusionGetTransformVars: function( values, varName, varNameHover, varNameParentHover ) {
		var transformVar      = this.fusionGetTransformStyle( values, 'regular' ),
			transformVarHover = this.fusionGetTransformStyle( values, 'hover' ),
			transformVars     = '';

		if ( transformVar ) {
			transformVars += varName + ':' + transformVar + ';';
		}
		if ( transformVarHover ) {
			transformVars += varNameHover + ':' + transformVarHover + ';';
			if ( 'parent' === values.transform_hover_element ) {
				transformVars += varNameParentHover + ':' + transformVarHover + ';';
			}
		}
	...
	/**
	 * Generates Transition style element.
	 *
	 * @since 3.9
	 * @param {Object} values - The values.
	 * @return {String}
	 */
	fusionGetTransitionStyle: function( values ) {
		const transition_duration = values.transition_duration + 'ms';
		const transition_easing   = 'custom' === values.transition_easing ? values.transition_custom_easing : values.transition_easing;
		const transition_style    = 'filter ' + transition_duration + ' ' + transition_easing + ', transform ' + transition_duration + ' ' + transition_easing + ', background-color ' + transition_duration + ' ' + transition_easing + ', border-color ' + transition_duration + ' ' + transition_easing;
	...
	fusionGetPattern( name, color, style ) {
		style = style || 'default';
		color = '' !== color ? jQuery.AWB_Color( color ).toRgbaString() : 'rgba(0,0,0,0.3)';
		const patterns = {
			'abstract': {
	 ...
	 * @return {String}
	 */
	fusionGetBackgroundSliderElement: function( self, type ) {
		...
		const images = '' !== values.background_slider_images ? values.background_slider_images.split( ',' ) : [];
		const attributes = [];

		attributes.push( `data-type="${type}"` );

		if ( '' !== values.background_slider_loop ) {
			attributes.push( `data-loop="${values.background_slider_loop}"` );
		}

		if ( '' !== values.background_slider_animation ) {
			attributes.push( `data-animation="${values.background_slider_animation}"` );
		}

		if ( '' !== values.background_slider_slideshow_speed ) {
			attributes.push( `data-slideshow-speed="${values.background_slider_slideshow_speed}"` );
		}

		if ( '' !== values.background_slider_animation_speed ) {
			attributes.push( `data-animation-speed="${values.background_slider_animation_speed}"` );
		}

		if ( '' !== values.background_slider_direction ) {
			attributes.push( `data-direction="${values.background_slider_direction}"` );
		}

		if ( 'yes' === values.background_slider_pause_on_hover ) {
			attributes.push( `data-pause_on_hover="${values.background_slider_pause_on_hover}"` );
		}

		let inline_style = '';
		if ( '' !== values.background_slider_position ) {
			inline_style += '--awb-image-position:' + values.background_slider_position + ';';
		}

		if ( '' !== values.background_slider_blend_mode ) {
			inline_style += `mix-blend-mode: ${values.background_slider_blend_mode};`;
		}

		if ( '' !== inline_style ) {
			attributes.push( `style="${inline_style}"` );
		}

		let element = '';
		if ( 'column' === type ) {
			let column_inline_style = '';
			if ( '' !== values.background_slider_blend_mode ) {
				let background_color_var = 'var(--awb-bg-color)';
				let background_image_var = 'var(--awb-bg-image)';
				const hover_or_link = ( 'none' !== values.hover_type && '' !== values.hover_type ) || '' !== values.link;
				...
				let background_image = 'background-image: ' + background_image_var + ';';
				if ( values.background_image ) {
					background_image = 'background-image:' + background_image_var + ', url(' + values.background_image + ');';
				}
	 ...
	 */
	getGradientString: function( values, type, size ) {
		var gradientString          = '',
		...
		if ( isGradientColor ) {
			...
			case 'parallax':
				...
				if ( size ) {
					bgKey = 'background_image_' + size;
				}

				if ( 'yes' !== values.fade ) {
					gradientString += ',url(\'' + values[ bgKey ] + '\')';
				}
				break;
			case 'fade':
			case 'column':
	 ...
	 * @return {string} - Generated string.
	 */
	getGradientFontString: function( values, valueOnly ) {
		var gradientString          = '',
	 ...
	 * @return {string} - The generated CSS.
	 */
	awbGetBoxShadowCssVar: function( varName, values ) {
		var style = '';
