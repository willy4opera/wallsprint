	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.fusion_title = FusionPageBuilder.ElementView.extend( {
			...
			onInit: function() {
				...
				this.listenTo( FusionEvents, 'fusion-content-changed', this.refreshJs );
			},
			...
			validateValues: function( values, extras ) {
				...
				if ( 'on' === values.loop_animation ) {
					values.loop_animation = 'loop';
				}
				if ( 'off' === values.loop_animation ) {
					values.loop_animation = 'once';
				}

				if ( 'highlight' === values.title_type && 'yes' === values.highlight_smudge_effect ) {
					const color = values.highlight_color ? values.highlight_color : extras.primary_color,
						alpha = jQuery.AWB_Color( color ).alpha();

					values.highlight_color_trans = jQuery.AWB_Color( color ).alpha( 0 ).toVarOrRgbaString();
					values.highlight_color_min   = jQuery.AWB_Color( color ).alpha( alpha * 0.15 ).toVarOrRgbaString();
					values.highlight_color_inter = jQuery.AWB_Color( color ).alpha( alpha * 0.3 ).toVarOrRgbaString();
					values.highlight_color_max   = jQuery.AWB_Color( color ).alpha( alpha * 0.9 ).toVarOrRgbaString();
				}
			...
			getStyleVars: function() {
				...
				if ( 'highlight' === this.values.title_type ) {
					...
					cssVars.push( 'highlight_animation_duration' );
					cssVars.push( 'highlight_width' );

					if ( 'marker' === this.values.highlight_effect && 'yes' === this.values.highlight_smudge_effect ) {
						cssVars.push( 'highlight_color_trans' );
						cssVars.push( 'highlight_color_min' );
						cssVars.push( 'highlight_color_inter' );
						cssVars.push( 'highlight_color_max' );
					}
				}

				if ( 'scroll_reveal' === this.values.title_type && ! this.isDefault( 'animated_text_color' ) ) {
					cssVars.push( 'animated_text_color' );
				}
			...
			buildAttr: function( values ) {
				...
				if ( 'text' !== values.title_type && '' !== values.loop_animation ) {
					attr[ 'class' ] += ' fusion-animate-' + values.loop_animation;
				}
				...
				if ( 'scroll_reveal' === values.title_type && values.scroll_reveal_effect ) {
					attr[ 'class' ]                        += ' awb-title__scroll-reveal--' + values.scroll_reveal_effect;
					attr[ 'data-scroll-reveal-above-fold' ] = 'yes' === values.scroll_reveal_above_fold ? 1 : 0;
					attr[ 'data-scroll-reveal-effect' ]     = values.scroll_reveal_effect;
					attr[ 'data-scroll-reveal-basis' ]      = values.scroll_reveal_basis;
					attr[ 'data-scroll-reveal-behavior' ]   = values.scroll_reveal_behavior;
					attr[ 'data-scroll-reveal-duration' ]   = parseFloat( values.scroll_reveal_duration ) / 1000;
					attr[ 'data-scroll-reveal-stagger' ]    = parseFloat( values.scroll_reveal_stagger ) / 1000;
					attr[ 'data-data-scroll-reveal-delay' ] = parseFloat( values.scroll_reveal_delay ) / 1000;
					attr[ 'data-scroll-reveal-counter' ]    = this.model.get( 'cid' );
				}
			...
			buildHeadingAttr: function( values ) {
				...
				if ( 'marquee' === values.title_type ) {
					...
					if ( 'yes' === values.marquee_mask_edges ) {
						headingAttr[ 'class' ] +=  ' awb-marquee-masked';
					}
				}
