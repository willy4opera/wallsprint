...
if ( fusion_is_element_enabled( 'fusion_gallery' ) ) {
	...
	if ( ! class_exists( 'FusionSC_FusionGallery' ) ) {
		...
		class FusionSC_FusionGallery extends Fusion_Element {
			 ...
			 * @access public
			 * @since 2.0.0
			 * @param 'parent'|'child' $context Can be either "parent" or "child".
			 * @return array
			 */
			...
			public function render( $args, $content = '' ) {
				...
				$defaults['limit'] = '0' == $defaults['limit'] ? get_option( 'posts_per_page' ) : $defaults['limit']; // phpcs:ignore Universal.Operators.StrictComparisons

				// Reset to default value if column = 0.
				if ( 0 === (int) $defaults['columns'] ) {
					$defaults['columns'] = $this->defaults['columns'];
				}
				...
				return apply_filters( 'fusion_element_gallery_parent_content', $html, $args );
			}
			...
			protected function get_style_variables() {
				...
				$styles = $this->get_css_vars_for_options( $css_vars_options ) . $this->get_font_styling_vars( 'caption_title_font' ) . $this->get_font_styling_vars( 'caption_text_font' ) . $this->get_custom_css_vars( $custom_vars );
	...
	fusion_builder_map(
		fusion_builder_frontend_data(
			...
			[
				...
				'sortable'        => false,
				'help_url'        => 'https://avada.com/documentation/gallery-element/',
				'subparam_map'    => [
					/* Caption title */
				...
				'params'          => [
					...
					[
						...
						'dependency'       => [
							[
								'element'  => 'parent_dynamic_content',
								'operator' => 'is_empty',
							],
						],
					],
					[
						...
						'description' => __( 'Defines how items should be ordered. <strong>NOTE:</strong> In the case of order by <strong>ASC</strong> and <strong>RAND</strong>, pagination in Live Builder will not work the same as the regular front-end.', 'fusion-builder' ),
						'default'     => 'desc',
						'value'       => [
					...
					[
						...
						'value'       => $fusion_settings->get( 'gallery_columns' ),
						'min'         => '0',
						'max'         => '6',
						'step'        => '1',
						'responsive'  => [
