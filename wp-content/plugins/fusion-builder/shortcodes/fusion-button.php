	...
	if ( ! class_exists( 'FusionSC_Button' ) ) {
		...
		class FusionSC_Button extends Fusion_Element {
			...
			public function __construct() {
				...
				add_filter( 'fusion_attr_button-shortcode-hover-icon', [ $this, 'hover_icon_attr' ] );
				add_filter( 'fusion_attr_button-shortcode-button-text', [ $this, 'button_text_attr' ] );
				add_filter( 'fusion_attr_button-shortcode-button-hover-text', [ $this, 'button_hover_text_attr' ] );
			...
			public function render( $args, $content = '' ) {
				...
				$this->args['button_text'] = $content;

				$icon_html           = '';
				$hover_icon_html     = '';
				$icon_and_hover_html = '';
				if ( $this->args['icon'] || ( 'yes' === $this->args['enable_hover_text_icon'] && $this->args['hover_icon'] ) ) {
					if ( $this->args['icon'] ) {
						$icon_html = '<i ' . FusionBuilder::attributes( 'button-shortcode-icon' ) . '></i>';
					}

					if ( 'yes' === $this->args['enable_hover_text_icon'] ) {
						
						if ( $this->args['hover_icon'] ) {
							$hover_icon_html = '<i ' . FusionBuilder::attributes( 'button-shortcode-hover-icon' ) . '></i>';
						}

						if ( 'icon_position' === $this->args['hover_transition'] ) {
							$icon_and_hover_html = $icon_html . $hover_icon_html;
						} else {
							$icon_and_hover_html = '<div class="awb-button__hover-content">' . $icon_html . $hover_icon_html . '</div>';
						}
					} else {
						$icon_and_hover_html = $icon_html;
					...
					if ( 'yes' === $this->args['icon_divider'] ) {
						$icon_and_hover_html = '<span ' . FusionBuilder::attributes( 'button-shortcode-icon-divder' ) . '>' . $icon_and_hover_html . '</span>';
					}
				}

				$button_base       = do_shortcode( $content );
				$button_text       = '<span ' . FusionBuilder::attributes( 'button-shortcode-button-text' ) . '>' . $button_base . '</span>';
				$button_hover_text = '';

				if ( 'yes' === $this->args['enable_hover_text_icon'] ) {
					$button_hover_base = do_shortcode( $this->args['hover_text'] );
					$button_hover_text = '<span ' . FusionBuilder::attributes( 'button-shortcode-button-hover-text' ) . '>' . $button_hover_base . '</span>';
					$centered          = '' === $icon_html &&  '' === $hover_icon_html ? ' awb-button__hover-content--centered' : '';

					if ( strlen( $button_base ) < strlen( $button_hover_base ) ) {
						$button_and_hover_html = '<div class="awb-button__hover-content awb-button__hover-content--default' . esc_attr( $centered ) . '">' . $button_hover_text . $button_text . '</div>';
					} else {
						$button_and_hover_html = '<div class="awb-button__hover-content awb-button__hover-content--default' . esc_attr( $centered ) . '">' . $button_text . $button_hover_text . '</div>';
					}

					// No button texts are set.
					if ( ( $this->args['icon'] || $this->args['hover_icon'] ) && ! $content && ! $this->args['hover_text'] ) {
						$icon_and_hover_html   = '<div class="awb-button__hover-content awb-button__hover-content--default awb-button__hover-content--centered">' . $icon_html . $hover_icon_html . '</div>';
						$button_and_hover_html = '';
					}

					// Icon, but no hover icon, no button text, but hover text.
					if ( $this->args['icon'] && ! $this->args['hover_icon'] && ! $content && $this->args['hover_text'] ) {
						$icon_and_hover_html   = '';
						$button_and_hover_html = '<div class="awb-button__hover-content awb-button__hover-content--reversed awb-button__hover-content--centered">' . $button_hover_text . $icon_html . '</div>';
					}

					// No icon, but hover icon, button text, but no hover text.
					if ( ! $this->args['icon'] && $this->args['hover_icon'] && $content && ! $this->args['hover_text'] ) {
						$icon_and_hover_html   = '';
						$button_and_hover_html = '<div class="awb-button__hover-content awb-button__hover-content--default awb-button__hover-content--centered">' . $button_text . $hover_icon_html . '</div>';						
					}
				} else if ( 'text_slide_up' === $this->args['hover_transition'] || 'text_slide_down' === $this->args['hover_transition'] ) {
					$centered = '' === $icon_html ? ' awb-button__hover-content--centered' : '';
					$button_and_hover_html = '<div class="awb-button-text-transition ' . esc_attr( $centered ) . '">' . $button_text . $button_text . '</div>';				
				} else {
					$button_and_hover_html = $button_text;
				}

				$inner_content = ( 'left' === $this->args['icon_position'] ) ? $icon_and_hover_html . $button_and_hover_html : $button_and_hover_html . $icon_and_hover_html;
			...
			protected function get_style_vars() {
				...
				if ( 'custom' === $this->args['color'] || 'default' === $this->args['color'] || false !== strpos( $this->args['color'], 'scheme-' ) ) {
					if ( ! $this->is_default( 'bevel_color' ) ) {
						$custom_vars['button_bevel_color'] = $this->args['bevel_color'];
					}
					if ( ! $this->is_default( 'bevel_color_hover' ) ) {
						$custom_vars['button_bevel_color_hover'] = $this->args['bevel_color_hover'];
					}

					if ( 'default' !== $this->args['color'] ) {

						$custom_vars['button_accent_color'] = $this->args['old_text_color'] ? $this->args['old_text_color'] : $this->args['accent_color'];
						if ( '' !== $this->args['border_color'] && ! $this->is_default( 'border_color' ) ) {
							$custom_vars['button_border_color'] = $this->args['border_color'];
						}
						...
						} elseif ( '' !== $this->args['accent_color'] && ! $this->is_default( 'accent_color' ) ) {
							$custom_vars['button_accent_hover_color'] = $this->args['accent_color'];
						}

						if ( '' !== $this->args['border_hover_color'] ) {
							$custom_vars['button_border_hover_color'] = $this->args['border_hover_color'];
						} elseif ( '' !== $this->args['accent_color'] ) {
							$custom_vars['button_border_hover_color'] = $this->args['accent_color'];
						}
					}

					if ( '' !== $this->args['border_width'] && 'custom' === $this->args['color'] && ! $this->args['default_border_width'] ) {
						$custom_vars['button_border_width-top']    = $this->args['border_top'];
						$custom_vars['button_border_width-right']  = $this->args['border_right'];
						$custom_vars['button_border_width-bottom'] = $this->args['border_bottom'];
						$custom_vars['button_border_width-left']   = $this->args['border_left'];
					}

					if ( ! $this->is_default( 'border_radius_top_left' ) ) {
						$custom_vars['button-border-radius-top-left'] = $this->args['border_radius_top_left'];
					}
					if ( ! $this->is_default( 'border_radius_top_right' ) ) {
						$custom_vars['button-border-radius-top-right'] = $this->args['border_radius_top_right'];
					}
					if ( ! $this->is_default( 'border_radius_bottom_right' ) ) {
						$custom_vars['button-border-radius-bottom-right'] = $this->args['border_radius_bottom_right'];
					}
					if ( ! $this->is_default( 'border_radius_bottom_left' ) ) {
						$custom_vars['button-border-radius-bottom-left'] = $this->args['border_radius_bottom_left'];
					}

					if ( 'default' !== $this->args['color'] ) {
						if ( ! $this->is_default( 'linear_angle' ) ) {
							$custom_vars['button_gradient_angle'] = (float) $this->args['linear_angle'] . 'deg';
						}
						if ( ! $this->is_default( 'gradient_start_position' ) ) {
							$custom_vars['button_gradient_start'] = (float) $this->args['gradient_start_position'] . '%';
						}
						if ( ! $this->is_default( 'gradient_end_position' ) ) {
							$custom_vars['button_gradient_end'] = (float) $this->args['gradient_end_position'] . '%';
						}
						...
						if ( $this->args['gradient_colors'] ) {
							...
							if ( 1 === count( $grad_colors ) || empty( $grad_colors[1] ) || $grad_colors[0] === $grad_colors[1] ) {
								$custom_vars['button_gradient_top_color']    = $grad_colors[0];
								$custom_vars['button_gradient_bottom_color'] = $grad_colors[0];
							} else {
								$custom_vars['button_gradient_top_color']    = $grad_colors[0];
								$custom_vars['button_gradient_bottom_color'] = $grad_colors[1];
								$this->args['has_gradient']                  = true;

								if ( 'linear' !== $this->args['gradient_type'] ) {
									$custom_vars['button_gradient'] = 'radial-gradient(circle at ' . $this->args['radial_direction'] . ',' . $grad_colors[0] . ' ' . $this->args['gradient_start_position'] . '%,' . $grad_colors[1] . ' ' . $this->args['gradient_end_position'] . '%)';
								}
							}
						}
						...
						if ( $this->args['gradient_hover_colors'] ) {
							...
							if ( 1 === count( $grad_hover_colors ) || '' === $grad_hover_colors[1] || $grad_hover_colors[0] === $grad_hover_colors[1] ) {
								$custom_vars['button_gradient_top_color_hover']    = $grad_hover_colors[0];
								$custom_vars['button_gradient_bottom_color_hover'] = $grad_hover_colors[0];
							} else {
								$custom_vars['button_gradient_top_color_hover']    = $grad_hover_colors[0];
								$custom_vars['button_gradient_bottom_color_hover'] = $grad_hover_colors[1];
								$this->args['has_gradient']                        = true;

								if ( 'linear' !== $this->args['gradient_type'] ) {
									$custom_vars['button_gradient_hover'] = 'radial-gradient(circle at ' . $this->args['radial_direction'] . ',' . $grad_hover_colors[0] . ' ' . $this->args['gradient_start_position'] . '%,' . $grad_hover_colors[1] . ' ' . $this->args['gradient_end_position'] . '%)';
								}
							}
						}
					}
				...
				if ( ! $this->is_default( 'text_transform' ) && '' !== $this->args['text_transform'] ) {
					$custom_vars['button_text_transform'] = $this->args['text_transform'];
				}

				if ( '' === $this->args['size'] ) {
					if ( ! $this->is_default( 'font_size' ) ) {
						$custom_vars['button_font_size'] = fusion_library()->sanitize->get_value_with_unit( $this->args['font_size'] );
					}

					if ( ! $this->is_default( 'line_height' ) ) {
						$custom_vars['button_line_height'] = $this->args['line_height'];
					}

					if ( ! $this->is_default( 'padding_top' ) ) {
						$custom_vars['button_padding-top'] = fusion_library()->sanitize->get_value_with_unit( $this->args['padding_top'] );
					}
					if ( ! $this->is_default( 'padding_right' ) ) {
						$custom_vars['button_padding-right'] = fusion_library()->sanitize->get_value_with_unit( $this->args['padding_right'] );
					}
					if ( ! $this->is_default( 'padding_bottom' ) ) {
						$custom_vars['button_padding-bottom'] = fusion_library()->sanitize->get_value_with_unit( $this->args['padding_bottom'] );
					}
					if ( ! $this->is_default( 'padding_left' ) ) {
						$custom_vars['button_padding-left'] = fusion_library()->sanitize->get_value_with_unit( $this->args['padding_left'] );
					}
				}

				if ( ! $this->is_default( 'letter_spacing' ) ) {
					$custom_vars['button_typography-letter-spacing'] = fusion_library()->sanitize->get_value_with_unit( $this->args['letter_spacing'] );
				}

				$text_styles = Fusion_Builder_Element_Helper::get_font_styling( $this->args, 'button_font', 'array' );
				foreach ( $text_styles as $rule => $value ) {
					$custom_vars[ 'button_typography-' . $rule ] = $value;
				}

				if ( ! $this->is_default( 'margin_top' ) ) {
					$custom_vars['button_margin-top'] = fusion_library()->sanitize->get_value_with_unit( $this->args['margin_top'] );
				}
				if ( ! $this->is_default( 'margin_right' ) ) {
					$custom_vars['button_margin-right'] = fusion_library()->sanitize->get_value_with_unit( $this->args['margin_right'] );
				}
				if ( ! $this->is_default( 'margin_bottom' ) ) {
					$custom_vars['button_margin-bottom'] = fusion_library()->sanitize->get_value_with_unit( $this->args['margin_bottom'] );
				}
				if ( ! $this->is_default( 'margin_left' ) ) {
					$custom_vars['button_margin-left'] = fusion_library()->sanitize->get_value_with_unit( $this->args['margin_left'] );
				}

				return $this->get_custom_css_vars( $custom_vars, false );
			}
			...
			public function attr() {
				...
				$color_type = $this->args['color'];
				if ( 'custom' === $this->args['color'] ) {
					$color_type = 'default';
				}

				$attr['class'] = 'fusion-button button-' . $this->args['type'] . ' ' . $size . ' button-' . $this->args['color'] . ' fusion-button-' . $color_type . ' button-' . $this->element_id . ' ' . $stretch . ' ' . $type;
				...
				if ( 'none' !== $this->args['hover_transition'] ) {
					$attr['data-hover'] = $this->args['hover_transition'];

					if ( 'icon_position' === $this->args['hover_transition'] && $this->args['icon'] ) {
						$attr['class'] .= ' awb-b-icon-pos-' . $this->args['icon_position'];
					}
				}
			...
			public function icon_attr() {
				return $this->get_icon_attributes();
			}

			/**
			 * Builds the hover icon attributes array.
			 *
			 * @access public
			 * @since 1.0
			 * @return array
			 */
			public function hover_icon_attr() {
				return $this->get_icon_attributes( 'hover' );
			}			

			/**
			 * Builds the icon attribute array, depending on the icon state.
			 *
			 * @access public
			 * @since 3.12
			 ...
			 * @return array
			 */			
			public function get_icon_attributes( $state = 'default' ) {
				$icon        = 'default' === $state ? $this->args['icon'] : $this->args['hover_icon'];
				$button_text = 'default' === $state ? $this->args['button_text'] : $this->args['hover_text'];
				$attr        = [
					'class'       => fusion_font_awesome_name_handler( $icon ),
					'aria-hidden' => 'true',
				];

				$attr['class'] .= ' awb-button__icon awb-button__icon--' . $state;

				if ( '' === $button_text ) {
					$attr['class'] .= ' awb-button__icon--no-margin';
				}
			...
			public function button_text_attr() {
				return $this->get_button_text_attribute();
			}


			/**
			 * Builds the hover text attributes array.
			 *
			 * @access public
			 * @since 1.0
			 * @return array
			 */
			public function button_hover_text_attr() {
				return $this->get_button_text_attribute( 'hover' );
			}
			 ...
			 * @return array
			 */			
			public function get_button_text_attribute( $state = 'default' ) {
				$attr = [
					'class' => 'fusion-button-text',
				];

				$attr['class'] .= ' awb-button__text awb-button__text--' . $state;

				if ( $this->args['icon'] && 'yes' === $this->args['icon_divider'] ) {
					$attr['class'] .= ' fusion-button-text-' . $this->args['icon_position'];
				}
	...
	fusion_builder_map(
		fusion_builder_frontend_data(
			...
			[
				...
				'help_url'      => 'https://avada.com/documentation/button-element/',
				'inline_editor' => true,
				'subparam_map'  => [
				...
				'params'        => [
					...
					[
						...
						'description'  => esc_attr__( 'Add the text that will display on the button.', 'fusion-builder' ),
						'dynamic_data' => true,
					],
					[
						'type'        => 'radio_button_set',
						'heading'     => esc_attr__( 'Enable Button Hover Text / Icon', 'fusion-builder' ),
						'description' => esc_attr__( 'Turn on to enable independent hover text and icon options.', 'fusion-builder' ),
						'param_name'  => 'enable_hover_text_icon',
						'default'     => 'no',
						'value'       => [
							'yes' => esc_attr__( 'On', 'fusion-builder' ),
							'no'  => esc_attr__( 'Off', 'fusion-builder' ),
						],
					],
					[
						'type'         => 'textfield',
						'heading'      => esc_attr__( 'Button Hover Text', 'fusion-builder' ),
						'param_name'   => 'hover_text',
						'value'        => '',
						'description'  => esc_attr__( 'Add the text that will display on button hover.', 'fusion-builder' ),
						'dynamic_data' => true,
						'dependency'    => [
							[
								'element'  => 'enable_hover_text_icon',
								'value'    => 'yes',
								'operator' => '==',
							],
						],
					...
					[
						'type'         => 'textfield',
						'heading'      => esc_attr__( 'Button Title / ARIA Label Attribute', 'fusion-builder' ),
						'param_name'   => 'title',
						'value'        => '',
						'description'  => esc_attr__( 'Set a title and aria-label attribute for the button.', 'fusion-builder' ),
						'dynamic_data' => true,
					],
					[
						'type'        => 'radio_button_set',
					...
					[
						...
						'description' => esc_attr__( 'Turn on for accessibility reasons, if your button does not use standard link behavior, but triggers an action like opening an Off-Canvas or a Modal.', 'fusion-builder' ),
					...
					[
						...
						'connect-state' => [ 'button_gradient_bottom_color', 'accent_color', 'border_color' ],
					],
					[
						'type'          => 'colorpickeralpha',
						...
						'connect-state' => [ 'button_gradient_top_color', 'accent_color', 'border_color' ],
					],
					[
						'type'        => 'range',
					...
					[
						...
						'connect-state' => [ 'button_gradient_top_color', 'button_gradient_bottom_color', 'border_color' ],
					],
					[
						'type'        => 'radio_button_set',
					...
					[
						...
						'connect-state' => [ 'button_gradient_top_color', 'button_gradient_bottom_color', 'accent_color' ],
					...
					[
						'type'         => 'iconpicker',
						'heading'      => esc_attr__( 'Hover Icon', 'fusion-builder' ),
					...
					[
						'type'        => 'select',
						'heading'     => esc_attr__( 'Hover Transition', 'fusion-builder' ),
						'description' => esc_attr__( 'Set the transition on hover. Icon effects will only work when an icon is set, icon position switch will only work without icon dividers.', 'fusion-builder' ),
						'param_name'  => 'hover_transition',
						'default'     => 'none',
						'group'       => esc_attr__( 'Design', 'fusion-builder' ),
						'value'       => [
							'none'            => esc_attr__( 'None', 'fusion-builder' ),
							'text_slide_up'   => esc_attr__( 'Text Slide Up', 'fusion-builder' ),
							'text_slide_down' => esc_attr__( 'Text Slide Down', 'fusion-builder' ),
