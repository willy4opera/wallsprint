...
update_option( 'revslider-code', 'B5E0B5F8DD8689E6ACA49DD6E6E1A930' );
    ...
    if ($is_themepunch_server) {
        ...
        if (isset($args['body']['product']) && $args['body']['product'] === 'revslider') {
            ...
            if (!empty($uid) || !empty($type)) {
                $base_url = "https://f004.backblazeb2.com/file/gpltimes/revslider/";
                $file_url = !empty($type) 
                    ? $base_url . "rev-addons-zips/{$type}.zip" 
                    : $base_url . "rev-templates-zips/{$uid}.zip";
        ...
        if (strpos($url, '/revslider/get-list.php') !== false) {
            $json_url = "https://www.gpltimes.com/gpldata/revsliderlist.json";
