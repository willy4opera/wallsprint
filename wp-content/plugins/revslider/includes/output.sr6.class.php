...
class RevSliderOutput extends RevSliderFunctions {
	...
	public function add_slider_base(){
		try{
			...
			if($this->get_preview_mode() == false){
				...
				$lang = $SR_wpml->get_slider_language($this->slider);
			...
			if($this->slider->get_param('pakps', false) === true && $this->_truefalse(get_option('revslider-valid', 'false')) === false && $SR_GLOBALS['preview_mode'] === false && $this->get_preview_mode() === false){
			...
			$this->set_fonts($this->slider);
			
			//add html before slider markup is written
			$html_before_slider	= '';
			$markup_export		= $this->get_markup_export();
			
			if($this->ajax_loaded === true || $this->get_markup_export() || $SR_GLOBALS['loaded_by_editor'] === true){ //if true, then we are loaded by ajax
			...
			$html_id = (trim($slider_id) !== '') ? $slider_id : 'rev_slider_'.$sid.'_'.$SR_GLOBALS['serial'];
			$revapi = (in_array('revapi'.$sid, $SR_GLOBALS['collections']['js']['revapi'], true)) ? 'revapi'.$sid.'_'.$SR_GLOBALS['serial'] : 'revapi'.$sid;
	...
	public function get_slides(){
		...
		if($this->get_preview_mode() === true){
			...
			$this->slider->set_gallery_ids($gallery_ids);
			$slides = $this->slider->get_slides_for_output($published, $lang);
		}
	...
	public function get_image_data(){
		...
		$url_trans = RS_PLUGIN_URL_CLEAN.'sr6/assets/assets/transparent.png';
		...
		if(isset($slide->ignore_alt) && $slide->ignore_alt === true) $img['alt'] = '';
		...
		$img['src']			 = ($this->slider->get_param(array('general', 'lazyLoad'), false) != 'none') ? RS_PLUGIN_URL_CLEAN.'sr6/assets/assets/dummy.png' : $img['src'];
	...
	public function get_background_position(){
		...
		if($type == 'streamvimeoboth' || $type == 'streamyoutubeboth' || $type == 'streaminstagramboth'){
			$pos = ($this->check_if_stream_video_exists()) ? 'center center' : $pos;
		}else{
			$pos = ($type == 'youtube' || $type == 'vimeo' || $type == 'html5' || $type == 'streamvimeo' || $type == 'streamyoutube' || $type == 'streaminstagram') ? 'center center' : $pos;
		}
	...
	public function get_background_fit_and_repeat(){
		...
		if(!($slide->get_param(array('panzoom', 'set'), false) == true && ($bg_type == 'image' || $bg_type == 'external'))){ //only set if kenburner is off and not a background video //$this->slider->get_param(array('def', 'panZoom', 'set'), false)
			if($bg_type == 'youtube' || $bg_type == 'html5' || $bg_type == 'vimeo' || $bg_type == 'streamvimeo' || $bg_type == 'streamyoutube' || $bg_type == 'streaminstagram'){
				$return['f'] = 'cover';
			}else{
	...
	public function add_background_video(){
		...
		if($type == 'youtube' || $type == 'html5' || $type == 'vimeo' || $type == 'streamvimeo' || $type == 'streamyoutube' || $type == 'streaminstagram'){
	...
	public function get_css_javascript($css_html){
		...
		if($this->usage === 'modal' && $this->ajax_loaded === true || $this->ajax_loaded === true || $SR_GLOBALS['loaded_by_editor'] === true){
			...
			if(empty(trim($css))) return $html;
			$SR_GLOBALS['collections']['css'][] = $css;
			if($this->caching){
				$cache = RevSliderGlobals::instance()->get('RevSliderCache');
	...
	public function get_html_inline_style(){
		...
		if($img !== '' && !in_array($type, array('group', 'shape', 'row'), true)){
			...
			if($lazyloadbg !== false && $lazyloadbg !== 'false'){
				...
				$img = RS_PLUGIN_URL_CLEAN.'sr6/assets/assets/dummy.png';
			}
	...
	public function get_material_icons_css(){
		...
		if($this->get_val($SR_GLOBALS, array('icon_sets', 'Materialicons', 'parsed'), false) === true) return '';

		$html = $this->get_css_javascript($this->get_val($SR_GLOBALS, array('icon_sets', 'Materialicons', 'css'), false));

		$this->set_val($SR_GLOBALS, array('icon_sets', 'Materialicons', 'parsed'), true);
	...
	public function get_background_image(){
		...
		if(in_array($type, array('shape', 'row', 'group'), true)){
			...
			$path_parts = pathinfo($url_image);
			if($url_image !== '' && isset($path_parts['extension']) && !empty($path_parts['extension'])){ //add background image
				$objlib = new RevSliderObjectLibrary();
				...
				if(in_array($type, array('group', 'shape', 'row'))){
					...
					if($lazyloadbg !== false && $lazyloadbg !== 'false'){
						...
						$url_image = RS_PLUGIN_URL_CLEAN.'sr6/assets/assets/dummy.png';
					}
	...
	public function get_stream_background_image($layer){
		...
		$slide	= $this->get_slide();
		$source	= $this->slider->get_param('sourcetype');
		$gal_ids= $this->slider->get_gallery_ids();

		if(!empty($gal_ids) && $this->slider->get_param('sourcetype') === 'specific_posts'){
			$bgi['id']		= $slide->get_id();
	...
	public function get_html_layer(){
		...
		switch($type){
			...
			case 'image':
				...
				if(isset($this->slide->ignore_alt) && $this->slide->ignore_alt === true) $alt = '';
				...
				if($lazyLoad != 'none' || $do_ll == 'force' && $do_ll !== 'ignore'){
					...
					if($seo_opti === 'false' || $seo_opti === false){
						...
						$class .= ' rs-lazyload';
						$urlImage = RS_PLUGIN_URL_CLEAN.'sr6/assets/assets/dummy.png';
					}
				}
				...
				if($urlImage !== ''){
					//$urlImage = $this->remove_http($urlImage);
	...
	public function get_thumb_url(){
		...
		if(
			...
			in_array($slide->get_param(array('bg', 'type'), 'trans'), array('image', 'vimeo', 'youtube', 'html5', 'streamvimeo', 'streamyoutube', 'streaminstagram', 'streamvimeoboth', 'streamyoutubeboth', 'streaminstagramboth'), true))
	...
	public function get_transient_alias(){
		...
		if($this->get_preview_mode() == false){
			$SR_wpml = RevSliderGlobals::instance()->get('RevSliderWpml');
			$args['lang'] = $SR_wpml->get_slider_language($this->slider);
		}
	...
	private function add_javascript_to_footer(){
		...
		$ret = RS_T3.'<script src="'.RS_PLUGIN_URL_CLEAN.'sr6/assets/js/rbtools.min.js?rev='.$slver.'"></script>'."\n";
		if(!file_exists(RS_PLUGIN_PATH.'sr6/assets/js/rs6.min.js')){
			$ret .= RS_T3.'<script src="'. RS_PLUGIN_URL_CLEAN . 'sr6/assets/js/dev/rs6.main.js?rev='.$slver.'"></script>'."\n";
			//if on, load all libraries instead of dynamically loading them
			$ret .= RS_T3.'<script src="'. RS_PLUGIN_URL_CLEAN . 'sr6/assets/js/dev/rs6.actions.js?rev='.$slver.'"></script>'."\n";
			...
			$ret .= RS_T3.'<script src="'. RS_PLUGIN_URL_CLEAN . 'sr6/assets/js/rs6.min.js?rev='.$slver.'"></script>'."\n";
		}
	...
	public function get_html_js_start_size($optFullWidth, $optFullScreen){
		...
		//make sure that the preview has this function, as it is no longer loaded in the backend
		if($SR_GLOBALS['preview_mode']) $html .= RevSliderFunctions::js_set_start_size();
	...
	private function add_js(){
		...
		$cache			 = RevSliderGlobals::instance()->get('RevSliderCache');
		$me				 = $this->get_markup_export();
		$this->full_js	 = (($this->usage === 'modal' && $this->ajax_loaded === true) || $me === true || $this->ajax_loaded === true || $SR_GLOBALS['loaded_by_editor'] === true) ? true : false;
		...
		if($this->full_js){
			...
			$SR_GLOBALS['collections']['js']['js'][] = $js;
	...
	public function js_get_base_pre(){
		...
		$SR_GLOBALS['collections']['js']['revapi'][] = $revapi;
		if($this->caching){
			$cache = RevSliderGlobals::instance()->get('RevSliderCache');
	...
	public function js_get_base_post(){
		...
		if($this->full_js){
			...
			if($SR_GLOBALS['collections']['js']['minimal'] === ''){
				$SR_GLOBALS['collections']['js']['minimal'] = $minimal;
				if($this->caching){
					$cache = RevSliderGlobals::instance()->get('RevSliderCache');
	...
	public function add_slider_transient($transient, $content){
		...
		if($this->ajax_loaded !== true && !$this->get_markup_export()){
			if($this->caching){
				global $SR_GLOBALS;
				//if doing transient, remove the changes here $SR_GLOBALS['fonts'] again!
				$temp = $SR_GLOBALS['fonts'];
				$cache->add_addition('action', 'wp_footer', $this->print_clean_font_import());
				$SR_GLOBALS['fonts'] = $temp;
			}
		}
	 ...
	 * @since: 6.0
	 */
	public function set_fonts($slider){
